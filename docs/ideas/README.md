...
- **`/avut-idea`** — quick capture mid-session. 1-2 clarifying questions, then a short
  file with `## Idea` / `## Notes`.
- **`/avut-brainstorm [topic|slug]`** — a deliberate back-and-forth exploration ending
  ...
  the result on a `brainstorm/*` branch (see `.claude/skills/avut-brainstorm/SKILL.md`).
- **`/avut-review-ideas [filter]`** — appends a `## Review` section (feasibility,
